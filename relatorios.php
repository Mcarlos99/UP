...
try {
    ...
    $stmt = $db->prepare("
        ...
        AND empresa_id = ?
    ");
    $stmt->execute([$dataInicio, $dataFim, $empresaId]);
    ...
    $stmt = $db->prepare("
        ...
        AND empresa_id = ?
    ");
    $stmt->execute([$dataInicioAnterior, $dataFimAnterior, $empresaId]);
...
@media (max-width: 768px) {
    ...
    /* Filtro de período em coluna */
    .periodo-grid {
        grid-template-columns: 1fr !important;
    }
    
    /* Gráficos e análises em coluna única */
    .content-grid {
        grid-template-columns: 1fr !important;
    }
...
<div class="white-card mb-4 md:mb-6 p-3 md:p-4">
    <form method="GET" action="" class="periodo-grid grid grid-cols-1 md:grid-cols-5 gap-3 md:gap-4">
        <div class="md:col-span-2">
            <label class="block text-sm font-semibold text-gray-700 mb-2">Período de Análise</label>
            <select name="periodo" id="periodoSelect" onchange="toggleCustomDates()" class="w-full px-3 md:px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm md:text-base">
        ...
        <div id="customDates" class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4" style="display: <?php echo $periodo === 'personalizado' ? 'grid' : 'none'; ?>;">
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Data Início</label>
                <input type="date" name="data_inicio" value="<?php echo $dataInicio; ?>" class="w-full px-3 md:px-4 py-2 border border-gray-300 rounded-lg text-sm md:text-base">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Data Fim</label>
                <input type="date" name="data_fim" value="<?php echo $dataFim; ?>" class="w-full px-3 md:px-4 py-2 border border-gray-300 rounded-lg text-sm md:text-base">
            </div>
        </div>
        
        <div class="flex items-end">
            <button type="submit" class="w-full bg-purple-600 text-white px-4 md:px-6 py-2 rounded-lg hover:bg-purple-700 transition font-semibold text-sm md:text-base">
...
<div class="stats-grid grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 md:mb-8">
    ...
    <div class="stat-card bg-gradient-to-br from-yellow-400 to-orange-500 rounded-xl p-5 text-white shadow-lg hover:shadow-xl transition">
        <div class="h-full flex flex-col items-center justify-center text-center">
            ...
            <p class="stat-title text-yellow-100 text-sm md:text-base font-bold mb-3">Faturamento</p>
            <p class="stat-value text-2xl md:text-3xl font-bold mb-2"><?php echo formatarMoeda($faturamento); ?></p>
            <p class="stat-subtitle text-yellow-100 text-xs font-semibold flex items-center gap-1 justify-center">
    ...
    <div class="stat-card bg-gradient-to-br from-blue-500 to-cyan-500 rounded-xl p-5 text-white shadow-lg hover:shadow-xl transition">
        <div class="h-full flex flex-col items-center justify-center text-center">
            ...
            <p class="stat-title text-blue-100 text-sm md:text-base font-bold mb-3">Total de Pedidos</p>
            <p class="stat-value text-2xl md:text-4xl font-bold mb-2"><?php echo $totalPedidos; ?></p>
            <p class="stat-subtitle text-blue-100 text-xs font-semibold flex items-center gap-1 justify-center">
    ...
    <div class="stat-card bg-gradient-to-br from-green-500 to-emerald-500 rounded-xl p-5 text-white shadow-lg hover:shadow-xl transition">
        <div class="h-full flex flex-col items-center justify-center text-center">
            ...
            <p class="stat-title text-green-100 text-sm md:text-base font-bold mb-3">Ticket Médio</p>
            <p class="stat-value text-2xl md:text-3xl font-bold mb-2"><?php echo formatarMoeda($ticketMedio); ?></p>
            <p class="stat-subtitle text-green-100 text-xs font-semibold">Por Pedido</p>
        </div>
    </div>
    ...
    <div class="stat-card bg-gradient-to-br from-purple-500 to-pink-500 rounded-xl p-5 text-white shadow-lg hover:shadow-xl transition">
        <div class="h-full flex flex-col items-center justify-center text-center">
            ...
            <p class="stat-title text-purple-100 text-sm md:text-base font-bold mb-3">Lucro Líquido</p>
            <p class="stat-value text-2xl md:text-3xl font-bold mb-2"><?php echo formatarMoeda($lucroLiquido); ?></p>
            <p class="stat-subtitle text-purple-100 text-xs font-semibold">
...
<div class="content-grid grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 mb-6 md:mb-8">
    ...
    <div class="card white-card">
        ...
        <?php if (empty($vendasPorCategoria)): ?>
            <p class="text-gray-500 text-center py-6 md:py-8 text-sm md:text-base">Nenhuma venda no período</p>
        <?php else: ?>
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            ...
                            <div class="text-right">
                                <span class="text-gray-900 font-bold text-sm md:text-base"><?php echo formatarMoeda($venda['valor_total']); ?></span>
                                <span class="text-gray-600 text-xs ml-1 md:ml-2">(<?php echo number_format($percentual, 1); ?>%)</span>
                            </div>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2 md:h-3">
                            <div 
                                class="bg-gradient-to-r from-purple-500 to-pink-500 h-2 md:h-3 rounded-full transition-all"
    ...
    <div class="card white-card">
        ...
        <?php if (empty($statusPedidos)): ?>
            <p class="text-gray-500 text-center py-6 md:py-8 text-sm md:text-base">Nenhum pedido no período</p>
        <?php else: ?>
            <div class="space-y-3 md:space-y-4">
                <?php 
                $statusLabels = [
                ...
                ?>
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-gray-700 font-medium text-sm md:text-base"><?php echo $statusInfo['label']; ?></span>
                            <div class="text-right">
                                <span class="text-gray-900 font-bold text-sm md:text-base"><?php echo $status['quantidade']; ?> pedidos</span>
                                <span class="text-gray-600 text-xs ml-1 md:ml-2">(<?php echo number_format($percentual, 1); ?>%)</span>
                            </div>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2 md:h-3">
                            <div 
                                class="bg-gradient-to-r <?php echo $statusInfo['color']; ?> h-2 md:h-3 rounded-full transition-all"
                                style="width: <?php echo $percentual; ?>%"
                            ></div>
                        </div>
...
<div class="content-grid grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 mb-6 md:mb-8">
    ...
    <div class="card white-card">
        <h3 class="text-lg md:text-xl font-bold text-gray-900 mb-4 md:mb-6">
            <i class="fas fa-trophy text-yellow-600"></i> Top 10 Produtos
        </h3>
        
        <?php if (empty($produtosMaisVendidos)): ?>
            <p class="text-gray-500 text-center py-6 md:py-8 text-sm md:text-base">Nenhum produto vendido</p>
        <?php else: ?>
            <div class="space-y-2 md:space-y-3">
                <?php foreach ($produtosMaisVendidos as $index => $produto): ?>
                    <div class="list-item top-item flex items-center justify-between p-3 md:p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition">
                        <div class="item-info flex items-center gap-2 md:gap-3 flex-1 min-w-0">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="text-xs md:text-sm text-gray-600"><?php echo htmlspecialchars($produto['categoria']); ?></p>
                            </div>
                        </div>
                        <div class="item-valor text-right flex-shrink-0">
                            <p class="font-bold text-purple-600 text-sm md:text-base"><?php echo formatarMoeda($produto['valor_total']); ?></p>
                            <p class="text-xs text-gray-600"><?php echo $produto['quantidade_vendida']; ?> un.</p>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
    ...
    <div class="card white-card">
        ...
        <?php if (empty($topClientes)): ?>
            <p class="text-gray-500 text-center py-6 md:py-8 text-sm md:text-base">Nenhum cliente no período</p>
        <?php else: ?>
            <div class="space-y-2 md:space-y-3">
                <?php foreach ($topClientes as $index => $cliente): ?>
                    <div class="list-item top-item flex items-center justify-between p-3 md:p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition">
                        <div class="item-info flex items-center gap-2 md:gap-3 flex-1 min-w-0">
                            <div class="w-8 h-8 md:w-10 md:h-10 bg-gradient-to-br from-blue-400 to-cyan-400 rounded-full flex items-center justify-center text-white font-bold text-sm md:text-base flex-shrink-0">
                                <?php echo $index + 1; ?>
                            </div>
                            <div class="min-w-0 flex-1">
                                <p class="font-semibold text-gray-900 text-sm md:text-base truncate"><?php echo htmlspecialchars($cliente['nome']); ?></p>
                                <p class="text-xs md:text-sm text-gray-600"><?php echo formatarTelefone($cliente['telefone']); ?></p>
                            </div>
                        </div>
                        <div class="item-valor text-right flex-shrink-0">
                            <p class="font-bold text-blue-600 text-sm md:text-base"><?php echo formatarMoeda($cliente['valor_total']); ?></p>
...
<div class="content-grid grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 mb-6 md:mb-8">
    ...
    <div class="card white-card">
        ...
        <?php if (empty($formasPagamento)): ?>
            <p class="text-gray-500 text-center py-6 md:py-8 text-sm md:text-base">Nenhum pagamento no período</p>
        <?php else: ?>
            <div class="space-y-3 md:space-y-4">
                <?php 
                ...
                ?>
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-gray-700 font-medium capitalize text-sm md:text-base">
                                <i class="fas fa-money-bill-wave text-green-500"></i> 
                                <?php echo ucfirst($forma['forma_pagamento']); ?>
                            </span>
                            <div class="text-right">
                                <span class="text-gray-900 font-bold text-sm md:text-base"><?php echo formatarMoeda($forma['valor_total']); ?></span>
                                <span class="text-gray-600 text-xs ml-1 md:ml-2">(<?php echo number_format($percentual, 1); ?>%)</span>
                            </div>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2 md:h-3">
                            <div 
                                class="bg-gradient-to-r from-green-500 to-emerald-500 h-2 md:h-3 rounded-full transition-all"
                                style="width: <?php echo $percentual; ?>%"
                            ></div>
                        </div>
                        <div class="text-xs text-gray-600 mt-1">
    ...
    <div class="card white-card">
        <h3 class="text-lg md:text-xl font-bold text-gray-900 mb-4 md:mb-6">
            <i class="fas fa-exclamation-triangle text-red-600"></i> Alertas de Estoque
        </h3>
        
        <?php if (empty($estoqueBaixo)): ?>
            <p class="text-green-600 text-center py-6 md:py-8 text-sm md:text-base">
                <i class="fas fa-check-circle text-3xl md:text-4xl mb-2 block"></i>
                Todos os produtos OK!
            </p>
        <?php else: ?>
            <div class="space-y-2 md:space-y-3">
                <?php foreach ($estoqueBaixo as $produto): ?>
                    <div class="list-item flex items-center justify-between p-3 md:p-4 bg-red-50 border-l-4 border-red-500 rounded-lg">
                        <div class="flex-1 min-w-0">
                            <p class="font-semibold text-gray-900 text-sm md:text-base truncate"><?php echo htmlspecialchars($produto['nome']); ?></p>
                            <p class="text-xs md:text-sm text-gray-600"><?php echo htmlspecialchars($produto['categoria']); ?></p>
                        </div>
                        <div class="text-right flex-shrink-0">
                            <p class="text-red-600 font-bold text-sm md:text-base"><?php echo $produto['estoque_atual']; ?> un.</p>
            ...
            <div class="mt-3 md:mt-4">
                <a href="produtos.php?estoque=baixo" class="text-purple-600 hover:text-purple-700 font-semibold text-xs md:text-sm">
...
<!-- Resumo Adicional -->
<div class="resumo-cards grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
    
    <!-- Card de Novos Clientes -->
    <div class="white-card">
        <div class="flex items-center justify-between mb-3 md:mb-4">
            <div>
                <p class="text-gray-600 text-xs md:text-sm">Novos Clientes</p>
                <p class="text-2xl md:text-3xl font-bold text-blue-600"><?php echo $novosClientes; ?></p>
            </div>
            <div class="bg-blue-100 p-2 md:p-3 rounded-lg">
                <i class="fas fa-user-plus text-blue-600 text-xl md:text-2xl"></i>
            </div>
        </div>
    ...
    <div class="white-card">
        <div class="flex items-center justify-between mb-3 md:mb-4">
            <div>
                <p class="text-gray-600 text-xs md:text-sm">Pedidos Concluídos</p>
                <?php
                $pedidosConcluidos = 0;
                foreach ($statusPedidos as $status) {
                ...
                ?>
                <p class="text-2xl md:text-3xl font-bold text-green-600"><?php echo number_format($taxaConclusao, 1); ?>%</p>
            </div>
            <div class="bg-green-100 p-2 md:p-3 rounded-lg">
                <i class="fas fa-check-double text-green-600 text-xl md:text-2xl"></i>
            </div>
        </div>
    ...
    <div class="white-card">
        <div class="flex items-center justify-between mb-3 md:mb-4">
            <div>
                <p class="text-gray-600 text-xs md:text-sm">Produtos Diferentes</p>
                <?php
                $totalProdutosDiferentes = count($produtosMaisVendidos);
                ?>
                <p class="text-2xl md:text-3xl font-bold text-purple-600"><?php echo $totalProdutosDiferentes; ?></p>
            </div>
            <div class="bg-purple-100 p-2 md:p-3 rounded-lg">
                <i class="fas fa-boxes text-purple-600 text-xl md:text-2xl"></i>
            </div>
        </div>
